...
(() => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const loggerMiddleware = (req, res, next) => {
        logger.log("info", `Incoming request, url ${req.url}, method: ${req.method}`);
        next();
    };
    ...
    app.post("/user/login", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        if (userJson.email == incomingUser.email &&
    ...
    app.get("/user/profile", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        helpers.prepareStandardHeaders(res);
        const userFile = JSON.parse(yield file.read(res.locals.decoded.payload, "json"));
        res.status(200).send({
            name: userFile.fname,
    ...
    app.delete("/posts", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        ...
        const postsFile = JSON.parse(yield file.read("posts", "json"));
        const userFile = JSON.parse(yield file.read(res.locals.decoded.payload, "json"));
        const result = JSON.stringify(postsFile.filter((el) => el.authorId != userFile.id));
        yield file.write(JSON.stringify(result), "posts", "json", "w");
        res.status(200).send("All user's posts have been deleted.");
        return;
    }));
    app.delete("/posts/byTitle", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        helpers.prepareStandardHeaders(res);
        const postsFile = JSON.parse(yield file.read("posts", "json"));
        const userFile = JSON.parse(yield file.read(res.locals.decoded.payload, "json"));
        const result = JSON.stringify(postsFile.filter((el) => el.authorId != userFile.id && el.title != req.params.title));
        yield file.write(JSON.stringify(result), "posts", "json", "w");
        res.status(200).send("Post deleted.");
        return;
    }));
    // receiving :title :description :createddate
    app.patch("/post", (req, res) => __awaiter(void 0, void 0, void 0, function* () {
        helpers.prepareStandardHeaders(res);
        const incomingPost = {
            id: req.body.id,
            title: req.body.title,
        ...
        const pos = postsFile.findIndex((el) => el.id == incomingPost.id);

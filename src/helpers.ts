...
interface Helpers {
  prepareStandardHeaders: (response:Response) => void,
  validateUser: (user:User) => boolean,
}

const helpers:Helpers = {
  prepareStandardHeaders: (response:Response) => {
  ...
  validateUser: (user:User) => {
    ...
    if (user["fname"] !== undefined) {
      if (/^[a-zA-Z]+$/.test(user["fname"])) result.push(true);
      else result.push(false);
    }

    if (user["lname"] !== undefined) {
      if (/^[a-zA-Z]+$/.test(user["lname"])) result.push(true);
      else result.push(false);
    }
